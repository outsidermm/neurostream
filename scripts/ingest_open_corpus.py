...
(~50-80 GB total across the five datasets, multiple hours). MOABB caches to
``~/mne_data`` so subsequent runs only re-harmonise. CPU time for
harmonisation alone is roughly 20-40 min depending on disk speed.
"""
...
def main(cfg: DictConfig) -> None:
    ...
    log.info(
        f"kept={len(manifest['recordings'])} "
